...
export default function AdminDashboard({ setIsAuthenticated, setIsAdmin, setStudentData }) {
  ...
  const loadData = async () => {
    try {
      if (USE_API) {
        let [s, f, c, m, msg, t] = await Promise.all([
        ...
        const apiCourses = Array.isArray(c) ? c : [];
        setStudents(s || []);
        setFaculty(f || []);
        ...
        try {
          const cs = await api.apiGet('/api/content-source');
          setContentSource(cs || []);
        } catch (csErr) {
          console.error('Failed to fetch content source:', csErr);
          setContentSource([]);
        }
      } else {
        ...
        // If it was the old nested object format, flatten it? The new format will be a flat array for easier querying.
        // Let's support both for transition, or assume flat array for this "Deep Link" update.
        let flatMaterials = [];
        if (Array.isArray(matRaw)) {
          flatMaterials = matRaw;
        } else {
          // Migration logic for old nested format if needed, but we'll stick to Array for new powerful data model
          flatMaterials = [];
        }
    ...
    <div className="admin-container">
      ...
      <div className={`admin-sidebar ${sidebarOpen ? 'open' : 'closed'}`}>
        ...
        <nav className="sidebar-nav">
          ...
          <SidebarItem id="advanced" icon={<FaRocket />} label="Advanced Learning" />
                  ...
                  <div className="modal-body">
                    ...
                    {modalType === 'student-view' && editItem && (
                      <div className="view-details">
                        <div className="detail-row"><strong>Name:</strong> {editItem.studentName}</div>
                        <div className="detail-row"><strong>ID:</strong> {editItem.sid}</div>
                        ...
                        <div className="detail-row"><strong>Branch:</strong> {editItem.branch}</div>
                        <div className="detail-row"><strong>Section:</strong> {editItem.section}</div>
